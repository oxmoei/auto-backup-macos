...
class BackupManager:
    ...
    def _contains_exclude_keyword(self, path):
        """检查路径中是否包含排除关键字
    ...
    def should_exclude_dir(self, path):
        ...
        # 检查路径中是否包含排除关键字
        if self._contains_exclude_keyword(abspath):
            if self.config.DEBUG_MODE:
                logging.debug(f"排除目录（包含关键字）: {path}")
            return True
        ...
        # 检查是否为配置中排除的目录
        exclude_names_lower = {name.lower() for name in self.config.EXCLUDE_INSTALL_DIRS}
        dir_name = os.path.basename(abspath)
        
        # 判断是否为主目录下的一级目录，且名称在排除列表中
        if abspath.startswith(home_dir):
            # 获取相对于主目录的路径
            try:
                rel_path = os.path.relpath(abspath, home_dir)
                # 如果是一级目录（不包含路径分隔符，且不是当前目录）
                if os.sep not in rel_path and rel_path != '.':
                    if dir_name.lower() in exclude_names_lower:
                        if self.config.DEBUG_MODE:
                            logging.debug(f"排除目录（一级目录）: {path}")
                        return True
            except ValueError:
                # 如果路径不在同一驱动器上，relpath 会抛出 ValueError
                pass
